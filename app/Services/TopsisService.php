...
final class TopsisService
{
    ...
    public function run(Assessment $a): void
    {
        try {
            ...
            $this->storeStep($a, 'MATRIX_X', [
                'rows' => $rows, 'cols' => $cols, 'X' => $X, 'algorithm' => 'TOPSIS',
            ]);
            ...
            $this->storeStep($a, 'NORMALIZED_R', [
                'rows' => $rows, 'cols' => $cols, 'R' => $R, 'denoms' => $denoms, 'skip_cols' => $skipCols,
            ]);
            ...
            $this->storeStep($a, 'WEIGHTED_Y', [
                'rows' => $rows, 'cols' => $cols, 'Y' => $Y, 'weights' => $W, 'skip_cols' => $skipCols,
            ]);

            // 5) A+ / A-
            [$Aplus, $Aminus] = $this->idealSolutions($Y, $cols, $types, $skipCols);
            $this->storeStep($a, 'IDEAL_SOLUTION', [
                'cols' => $cols, 'A_plus' => $Aplus, 'A_minus' => $Aminus, 'types' => $types, 'skip_cols' => $skipCols,
            ]);
            ...
            $this->storeStep($a, 'DISTANCES', [
                'rows' => $rows, 'S_plus' => $Splus, 'S_minus' => $Sminus,
    ...
    private function imputeMedianPerColumn(array $X, array $cols, array &$meta): array
    {
        ...
        for ($j=0; $j<$n; $j++) {
            ...
            if (count($nonNull) === 0) { $skip[$cols[$j]] = true; continue; }
            sort($nonNull);
    ...
    private function normalizeR(array $X): array
    {
        ...
        for ($i=0; $i<$m; $i++) {
            $row = [];
            for ($j=0; $j<$n; $j++) {
                if ($denoms[$j] == 0.0) { $row[] = 0.0; $skipCols[$j] = true; }
                else { $row[] = ($X[$i][$j] ?? 0.0) / $denoms[$j]; }
            }
            $R[] = $row;
        }
    ...
    {
        $m = count($Y); $n = $m ? count($Y[0]) : 0;
        $Aplus = array_fill(0,$n,0.0);
        $Aminus = array_fill(0,$n,0.0);

        for ($j=0; $j<$n; $j++) {
            if (in_array($j,$skipCols,true)) { $Aplus[$j]=0.0; $Aminus[$j]=0.0; continue; }
    ...
    private function distances(array $Y, array $Aplus, array $Aminus, array $cols, array $skipCols): array
    {
        ...
        $dPlus2 = []; $dMinus2 = []; // per i: [j => (Δ)^2]

        for ($i=0; $i<$m; $i++) {
            $sumP=0.0; $sumM=0.0;
            $dP=[]; $dM=[];
            for ($j=0; $j<$n; $j++) {
                if (in_array($j,$skipCols,true)) { $dP[$j]=0.0; $dM[$j]=0.0; continue; }
                $dp = ($Y[$i][$j] - $Aplus[$j]);
                $dm = ($Y[$i][$j] - $Aminus[$j]);
                $dp2 = $dp*$dp; $dm2 = $dm*$dm;
                $sumP += $dp2; $sumM += $dm2;
    ...
    private function shouldUseCompatibilityColumns(Assessment $a): bool
    {
        ...
        $compColumns = ['COMP_Fitness_vs_Gain', 'COMP_Exp_vs_Distance', 'COMP_Survival_vs_LandCover', 
                       'COMP_Team_vs_Support', 'COMP_Cardio_vs_Gain', 'COMP_Guide_vs_Slope'];
